...
import { NormalizedTrade, RawTrade } from "./types";
import { logger } from "./logger";

function normalizeTrade(raw: RawTrade, wallet: string): NormalizedTrade | null {
  const outcomeRaw = (raw.outcome || raw.side || "").toUpperCase();
  let outcome: NormalizedTrade["outcome"] = "UNKNOWN";
  if (outcomeRaw.includes("YES")) outcome = "YES";
  else if (outcomeRaw.includes("NO")) outcome = "NO";

  const sizeNum = Number(raw.size ?? raw.amount ?? 0) || 0;
  const priceNum = Number(raw.price ?? 0.5) || 0.5;
  const notional = sizeNum * priceNum;

  if (!raw.id || !wallet || notional < config.minTradeNotional) {
    return null;
  }
  ...
  async pollNewTrades(whaleSet: Set<string>): Promise<NormalizedTrade[]> {
    ...
    if (results.length) {
      logger.info(
        ...
        "Emitting new normalized whale trades"
      );
    }
    return results;
  }
